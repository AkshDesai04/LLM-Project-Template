"""
Live integration tests for OpenAI Embedding models across all supported file formats.
...
@pytest.mark.live
@pytest.mark.openai
def test_openai_embeddings_short_inputs():
...
@pytest.mark.live
@pytest.mark.openai
def test_openai_embeddings_large_inputs():
    for model in EMBEDDING_MODELS:
...
@pytest.mark.live
@pytest.mark.openai
def test_openai_embeddings_batch_and_options():
    model = "openai/text-embedding-3-small"
    batch_input = [SHORT_FORMAT_SAMPLES["txt"], SHORT_FORMAT_SAMPLES["md"]]
    vecs = run_embedding_test(model, batch_input, dimensions=768)
